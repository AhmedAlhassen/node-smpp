...
class Session extends EventEmitter {
  ...
  set auto_enquire_link_period(val) {
    console.log(`setting enquire_link interval to ${val}ms`);
    if (this._interval) clearInterval(this._interval);
    this._interval = setInterval(() => {
      console.log('sending enquire_link');
      this.enquire_link()
        .then((pdu) => {
          return console.log({pdu}, 'successfully sent enquire_link');
        })
        .catch((err) => {
  ...
  _extractPDUs() {
    ...
    while (!this.paused) {
      ...
      this._command_length = null;
      this.emit('pdu', pdu);
      console.log(`got pdu with sequence number ${pdu.sequence_number}`);
      this.emit(pdu.command, pdu);
      if (pdu.isResponse() && this._callbacks[pdu.sequence_number]) {
        console.log('got response pdu');
  ...
  send(pdu, sendCallback) {
    ...
    return new Promise((resolve, reject) => {
      if (!pdu.isResponse()) {
        ...
        console.log(`sending pdu with sequence number ${pdu.sequence_number}`);
        this._callbacks[pdu.sequence_number] = resolve;
      }
      console.log('sending....');
      this.socket.write(pdu.toBuffer(), () => {
        console.log('sent pdu');
        this.emit('send', pdu);
